...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      return new Response(
        JSON.stringify({ success: false, error: "Supabase environment variables are missing." }),
        {
          status: 500,
    ...
    if (callerError || !callerProfile) {
      return new Response(
        JSON.stringify({ success: false, error: "Caller profile lookup failed." }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (callerProfile.role !== "admin" && callerProfile.role !== "super_admin") {
      return new Response(
        JSON.stringify({ success: false, error: "Only admins can send parent credentials." }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (!resendResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: `Resend error: ${resendError}` }),
        {
          status: 502,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
