...
export async function getParentPortalBootstrap(
  ...
  if (!childrenResult.success || !childrenResult.data) {
    return {
      data: null,
      error: childrenResult.error ?? "Linked children not found.",
      success: false,
    };
  }
...
export async function getParentChildSnapshot(
  studentId: string,
): Promise<ApiResponse<ParentPortalChildSnapshot>> {
  if (!supabase) return SUPABASE_NOT_CONFIGURED;
  ...
  const [
    ...
    student.batch_id
      ? getStudentBatch(student.batch_id, student.institute_id)
      : Promise.resolve({
          data: null,
          error: null,
          success: true,
        } as ApiResponse<StudentBatchInfo | null>),
