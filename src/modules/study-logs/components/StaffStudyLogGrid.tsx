...
export function StaffStudyLogGrid({
  ...
  const filteredData = useMemo(() => {
    ...
    return reportData.filter((student) => student.student_name.toLowerCase().includes(query));
          ...
          <thead className="sticky top-0 z-20 bg-card border-b border-border">
            <tr>
              ...
              {days.map((day) => (
                <th
                  key={day.toISOString()}
                  className="p-4 text-center font-medium text-muted-foreground min-w-[120px]"
                >
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                onClick={() =>
                                  log && setSelectedLog({ studentName: student.student_name, log })
                                }
                              ...
                              {log ? (
                                <>
                                  ...
                                  <p className="text-[10px] text-muted-foreground pt-1 border-t border-border">
                                    Submitted:{" "}
                                    {format(parseISO(log.submitted_at), "MMM dd, hh:mm a")}
                                  </p>
                                </>
                              ) : (
                                <p className="text-xs text-muted-foreground">
                                  {status === "missing"
                                    ? "No log submitted for this date."
                                    : "Awaiting submission..."}
                                </p>
                              )}
                            </TooltipContent>
              ...
              <DialogHeader>
                <div className="flex items-center justify-between pr-8">
                  <div className="space-y-1">
                    ...
                    <DialogDescription>
                      Submitted by {selectedLog.studentName} for{" "}
                      {format(parseISO(selectedLog.log.log_date), "MMMM do, yyyy")}
                    </DialogDescription>
                  </div>
                  <Badge
                    variant={selectedLog.log.is_late ? "secondary" : "default"}
                    className={
                      selectedLog.log.is_late
